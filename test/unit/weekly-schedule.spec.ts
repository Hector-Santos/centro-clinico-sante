...
import { WeeklyScheduleService } from 'src/weekly_schedules/weekly_schedule.service';
import { CreateWeeklyScheduleDto } from 'src/weekly_schedules/dto/weekly_schedule.dto';
import {
  CreateWeeklyScheduleMock,
  UpdateWeeklyScheduleMock,
  WeeklyScheduleMock,
} from 'test/mocks/weekly_schedule.factory';
...
describe('WeeklyScheduleService', () => {
  ...
  it('should create successfully', async () => {
    const input: CreateWeeklyScheduleDto = CreateWeeklyScheduleMock();
  ...
  // FIND ONE (success)
  it('should return an weekly_schedule by ID', async () => {
    const target = WeeklyScheduleMock();
  ...
  // FIND ONE (fail)
  it('should throw if weekly_schedule is not found', async () => {
    const mockGet = jest.fn().mockResolvedValue({ exists: false });
    (mockCollection.doc as any) = jest.fn(() => ({ get: mockGet }));

    await expect(service.findOne('not-found')).rejects.toThrow(
      'WeeklySchedule not found',
    );
    expect(mockCollection.doc).toHaveBeenCalledWith('not-found');
  });

  // FIND ALL (success)
  it('should return all weekly_schedules', async () => {
    const weekly_schedules = [WeeklyScheduleMock(), WeeklyScheduleMock()];

    (mockCollection as any).get = jest.fn().mockResolvedValue({
      docs: weekly_schedules.map((a) => ({
        id: a.id,
        data: () => a,
      })),
    });
    ...
    // FIND ALL (fail)
    it('should throw if fetching weekly schedules fails', async () => {
      const getMock = jest
        .fn()
    ...
    expect(result).toEqual(weekly_schedules);
    expect(mockCollection.get).toHaveBeenCalledTimes(1);
  });

  // UPDATE (success)
  it('should update an weekly_schedule successfully', async () => {
    const mockUpdate = jest.fn().mockResolvedValue(undefined);
    (mockCollection.doc as any) = jest.fn(() => ({ update: mockUpdate }));

    const dto = UpdateWeeklyScheduleMock();
  ...
  it('should throw if update fails', async () => {
    ...
    await expect(
      service.update('some-id', UpdateWeeklyScheduleMock()),
    ).rejects.toThrow('Firestore failure');
    expect(mockCollection.doc).toHaveBeenCalledWith('some-id');
  });

  // DELETE (success)
  it('should delete an weekly_schedule by ID', async () => {
